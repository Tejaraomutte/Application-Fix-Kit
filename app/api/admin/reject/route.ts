...
export async function POST(req: Request) {
  ...
  try {
    ...
    const updated = await updateSubmission(reference, {
      ...
      verifiedBy: 'admin',
      notes: 'Payment rejected during manual verification.',
    });
    ...
    if (url) {
      const r = await fetch(url, {
        ...
        body: JSON.stringify({ reference, action: 'reject', verifiedAt: new Date().toISOString(), verifiedBy: 'admin' }),
      });
    ...
    return NextResponse.json({
      message: updated ? 'Payment rejected successfully.' : 'Payment reference not found. Check the submitted reference and try again.',
    });
  } catch {
